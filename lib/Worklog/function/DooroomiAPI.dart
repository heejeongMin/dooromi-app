...
class DooroomiAPI {
  ...
  static Future<bool> fetchPost(Worklog worklog) async {
    final response = http.post(Uri.parse(localhost + uri),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
    ...
    response
        ...
        .onError((error, stackTrace) => throw Exception('Fail to save'));
  ...
  static deleteWorklog(Worklog worklog, BuildContext context) {
    ...
    final queryParam = {'ids': worklog.worklogNumber.toString()};
    ...
    response.onError((error, stackTrace) => throw Exception('Fail to save'));
